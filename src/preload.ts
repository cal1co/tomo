...
contextBridge.exposeInMainWorld("electron", {
  ...
  send: (channel: string, data: any) => {
    const validChannels = ['show-main-app', 'quit-app', 'get-menu-items', 'sync-state', 'save-board-state'];
    if (validChannels.includes(channel)) {
      ipcRenderer.send(channel, data);
    }
  },
  receive: (channel: string, callback: (data: any) => void) => {
    const validChannels = ['update-menu-items', 'update-submenu', 'sync-state-update', 'perform-undo', 'show-message'];
    if (validChannels.includes(channel)) {
